...
public class HelloWorldServer {
	...
	private void start() throws IOException {
		...
		server = Grpc.newServerBuilderForPort(port, InsecureServerCredentials.create())
				...
				.addService(new MyService2Impl())
				.addService(new MyService3Impl())
				.build()
				.start();
	...
	static class MyService1Impl extends MyService1Grpc.MyService1ImplBase {
		public void function1Service1(MsgRequest req, StreamObserver<MsgReply> responseObserver) {
			MsgReply reply = MsgReply.newBuilder().setMessage(req.getMessage() + "(Server said: Hiya!)").build();
	...
	static class MyService2Impl extends MyService2Grpc.MyService2ImplBase {
		public void function1Service2(MsgRequest req, StreamObserver<MsgReply> responseObserver) {
			MsgReply reply = MsgReply.newBuilder().setMessage(req.getMessage() + "(Server said: grand)").build();
			responseObserver.onNext(reply);
			responseObserver.onCompleted();
		}
		
		public void function2Service2(MsgRequest req, StreamObserver<MsgReply> responseObserver) {
			MsgReply reply = MsgReply.newBuilder().setMessage(req.getMessage() + "(Server said: grand)").build();
			responseObserver.onNext(reply);
			responseObserver.onCompleted();
		}
	...
	static class MyService3Impl extends MyService3Grpc.MyService3ImplBase {
		public void function1Service3(MsgRequest req, StreamObserver<MsgReply> responseObserver) {
			MsgReply reply = MsgReply.newBuilder().setMessage("(Server said: What's the craic?)" + req.getMessage()).build();
			responseObserver.onNext(reply);
			responseObserver.onCompleted();
		}
	}
